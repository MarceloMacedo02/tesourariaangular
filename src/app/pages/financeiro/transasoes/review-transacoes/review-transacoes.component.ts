...
@Component({
  ...
  standalone: false,  // Este componente não é standalone, por isso precisa ser declarado no módulo
  template: `
    <div class="page-content">
      <!-- Start Breadcrumbs -->
      <app-breadcrumbs
        [title]="'Review de Transações'"
          ...
          <div class="col-12">
            <div class="card">
              ...
              <div class="card-body">
                ...
                <div class="table-responsive">
                  <table class="table table-striped table-nowrap">
                    ...
                    <tbody>
                      <tr
                        *ngFor="
                          let transacao of transacoesFiltradas;
                          trackBy: trackByFn
                        "
                      >
                        ...
                        <td>
                          <span
                            class="badge"
                            [ngClass]="getTipoClasse(transacao.tipo)"
                          >
                            {{
                        ...
                        <td>
                          <span
                            class="badge"
                            [ngClass]="getStatusClasse(transacao.lancado)"
                          >
                ...
                >
                  <ul class="pagination justify-content-center">
                    ...
                    <li
                      ...
                      *ngFor="let i of getRange(totalPages)"
                    >
                      <a
                        class="page-link"
                        href="javascript:void(0)"
                ...
                <div
                  *ngIf="transacoesFiltradas.length === 0 && !carregando"
                  class="text-center py-4"
                >
                  <i class="ri-bank-card-2-line display-4 text-muted"></i>
                  <h5 class="mt-3">Nenhuma transação encontrada</h5>
                  <p class="text-muted">
                    Não há transações correspondentes aos filtros aplicados.
                  </p>
                </div>
...
export class ReviewTransacoesComponent implements OnInit {
    ...
    this.transacoesOfxService
      ...
      .subscribe({
        next: (response: TransacaoResponse) => {
          ...
          this.transacoes = response.content.map(
            (transacao: any) =>
              ({
                ...transacao,
        ...
        error: (error: any) => {
          ...
          if (error.status === 403) {
            this.errors.push('Acesso negado: você não tem permissão para visualizar transações. Verifique suas credenciais.');
          } else if (error.status === 401) {
            this.errors.push('Sessão expirada: faça login novamente para continuar.');
          } else if (error.status === 0) {
            this.errors.push('Erro de conexão: não foi possível se conectar ao servidor. Verifique se o backend está rodando.');
          } else if (error.status === 404) {
            this.errors.push('Endpoint não encontrado: verifique se o backend está configurado corretamente.');
          } else {
            this.errors.push(
              error.error?.message || 'Erro ao carregar transações. Por favor, tente novamente.'
            );
          }
  ...
  calcularResumo(): void {
    ...
    this.totalReceitas = this.transacoes.filter(
      (t) => t.tipo === 'CREDITO'
    ).length;
    this.totalDespesas = this.transacoes.filter(
      (t) => t.tipo === 'DEBITO'
    ).length;
    this.totalPendentes = this.transacoes.filter(
      (t) => !this.isLancado(t.lancado)
    ).length;
  }
  ...
  getStatusClasse(lancado: 'LANCADO' | 'NAOLANCADO' | 'SIM' | 'NAO'): string {
    if (lancado === 'LANCADO' || lancado === 'SIM') {
      return 'bg-success';
    } else {
      return 'bg-warning';
    }
  ...
  getStatusTexto(lancado: 'LANCADO' | 'NAOLANCADO' | 'SIM' | 'NAO'): string {
    if (lancado === 'LANCADO' || lancado === 'SIM') {
      return 'Lançado';
    } else {
      return 'Pendente';
  ...
  private isLancado(lancado: 'LANCADO' | 'NAOLANCADO' | 'SIM' | 'NAO'): boolean {
    return lancado === 'LANCADO' || lancado === 'SIM';
  }
}
