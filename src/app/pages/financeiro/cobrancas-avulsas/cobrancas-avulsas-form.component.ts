...
import { CobrancasAvulsasService, Cobranca, NovaCobrancaAvulsa } from './cobrancas-avulsas.service';
import { ReferenciasFinanceirasService, Fornecedor, Rubrica, Socio } from '../../../services/referencias-financeiras.service';
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title mb-0">{{ isEdicao ? 'Editar Cobrança' : 'Nova Cobrança Avulsa' }}</h4>
              </div>
              <div class="card-body">
                <!-- Erros de validação -->
...
export class CobrancasAvulsasFormComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    const cobrancaData: NovaCobrancaAvulsa = {
      ...
      fornecedorId: this.selectedFornecedorId || formData.fornecedorId || undefined,
    ...
    if (this.isEdicao && this.cobrancaId) {
      ...
      this.cobrancasAvulsasService.createAvulsa(cobrancaData).subscribe({
        ...
        error: (error) => {
          ...
          alert('Erro ao criar cobrança avulsa: ' + (error.error?.message || 'Erro desconhecido'));
        }
      });
    }
  }
